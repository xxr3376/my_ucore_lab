...
load_icode(int fd, int argc, char **kargv) {
	...
	struct Page *page;
	struct elfhdr elf0;
	struct elfhdr* elf = &elf0;
	struct proghdr ph0;
	struct proghdr *ph = &ph0;
	...
	uintptr_t stack = USTACKTOP - argc * PGSIZE;
	char **uargv = (char **)(stack - argc * sizeof(char *));
	int i;
	for (i = 0; i < argc; i++){
		uargv = strcpy((char *) (stack + i * PGSIZE), kargv[i]);
	}
	stack = (uintptr_t) uargv - sizeof(int);
	*(int *) stack = argc;
